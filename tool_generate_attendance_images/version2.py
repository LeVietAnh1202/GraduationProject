...
def generate_images(uploaded_files, paths, root_dir):
    ...
    for path in paths:
        ...
        period = img_name[:2]  # Lấy 2 chữ số đầu của tên ảnh (xx)

        sample_image_key = f"{period}_{ma_sv_ten_sv}"
        sample_image_path = sample_images.get(sample_image_key)
        ...
        if sample_image_path:
            ...
            st.error(f"Không tìm thấy ảnh mẫu cho {sample_image_key}")
